...
class RobotState:
    ...
    def from_dict(cls, d: dict) -> RobotState:
        ...
        # gripper — may be a dict or a scalar
        gs: JsonDict = {}
        ...
        if gs_raw and isinstance(gs_raw, dict):
            ...
            if go is not None:
                ...
                gs = {"open": val > 0.5}
        finger_qpos = _to_float_list(d.get("gripper_qpos"))
        if finger_qpos:
            gs["finger_qpos"] = finger_qpos
            # Panda finger joints are symmetric around zero.  Sum of absolute
            # joint positions is the physical jaw aperture in metres and lets
            # Host diagnostics distinguish a fully closed empty grasp from
            # fingers stopped by an object.  This is measurement, not a contact
            # classifier.
            gs["aperture_m"] = sum(abs(value) for value in finger_qpos)
        finger_qvel = _to_float_list(d.get("gripper_qvel"))
        if finger_qvel:
            gs["finger_qvel"] = finger_qvel
        gripper_geometry = d.get("gripper_geometry")
        if isinstance(gripper_geometry, dict):
            # Static/live robot collision geometry is ordinary proprioception,
            # not privileged object state. Preserve it so visualization can
            # show the actual finger contact locations instead of inferring
            # them from the joint aperture.
            gs["geometry"] = _ensure_plain_dict(gripper_geometry)
