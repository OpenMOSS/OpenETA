...
class OpenAICompatiblePlannerBackend(PlannerBackend):
    ...
    def decide(self, request: PlannerBackendRequest) -> PlannerBackendResult:
        ...
        response, provider_attempts, retry_errors, final_error = self._request_with_retry(
            url,
            body,
            headers,
        )
        ...
        usage, usage_source, usage_estimator = _provider_or_estimated_usage(
            response.get("usage"),
            request_body=_usage_estimation_body(body),
            completion=content,
            model=self.config.model,
        )
        return PlannerBackendResult(
    ...
    ) -> tuple[JsonDict | None, int, list[JsonDict], Exception | None]:
        max_attempts = max(1, self.config.max_attempts)
        retry_errors: list[JsonDict] = []
        for attempt in range(1, max_attempts + 1):
            try:
                response = self.transport(url, body, headers, self.config.timeout_s)
                return response, attempt, retry_errors, None
                ...
                if attempt >= max_attempts or not _is_transient_provider_error(exc):
                    return None, attempt, retry_errors, exc
...
def _planner_user_content(
    ...
    for path_value in paths[: config.max_vision_images]:
...
def _provider_or_estimated_usage(
    ...
    *,
    ...
    derived_total = int(usage.get("prompt_tokens") or 0) + int(
