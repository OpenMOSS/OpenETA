...
class OpenEtaCli:
    ...
    def run(self) -> None:
        ...
        if not sys.stdin.isatty():
            print(
                Theme.warn(
                    "OpenETA CLI needs an interactive TTY. Run it with: uv run openeta"
                )
            )
            return
        while True:
        ...
        *,
        ...
        try:
            result = self.state.episode_runner.run(
                task=task,
                max_turns=max_turns,
                metadata={"source": "OpenEtaCli", "environment_mode": "tool_feedback"},
            )
        except KeyboardInterrupt:
            self._finish_agent_activity("interrupted")
            self._mark_current_episode_interrupted()
    ...
    def _print_sessions(self) -> None:
        ...
        if not isinstance(store, JsonMemoryStore):
            print(Theme.warn("No local JSON memory store is configured."))
            return
    ...
    def _resume_session(self, args: list[str]) -> None:
        ...
        self.state.current_task = runtime.memory.task or ""
        self.state.step_idx = 0
        self.state.continue_after_human = False
        self.state.episode_runner = None
        ...
        if runtime.memory.task:
            print(f"  task {runtime.memory.task}")
        location = _memory_location(runtime.memory)
    ...
    def _print_session(self) -> None:
        ...
        print(f"  session_id   {runtime.memory.session_id or '(not started)'}")
        print(f"  task         {runtime.memory.task or self.state.current_task or '(not started)'}")
    ...
    def _build_runtime(self) -> None:
        backend_config = OpenAICompatiblePlannerBackendConfig.from_provider_config(
            self.state.config
        )
        planner = ToolCallingPlanner(
            OpenAICompatiblePlannerBackend(backend_config),
            context_config=PlannerContextConfig(
                context_window_tokens=self.state.config.context_window_tokens,
                token_estimator_model=self.state.config.model,
            ),
        )
        self._refresh_mcp_registry()
        tools = _build_cli_tool_registry(self)
        memory = AgentMemory(store=JsonMemoryStore())
        self.state.runtime = OpenEtaAgentRuntime(
            planner=planner,
            tools=tools,
            memory=memory,
            pipeline=ActionPipeline(
                checker_subagents=_build_cli_checker_config(tools),
            ),
        )
        self._save_mcp_registry_to_memory()
        self._save_simulator_mcp_tool_catalog_to_memory()
    ...
    def _print_action_trace(self, action: EnvAction) -> None:
        ...
        if usage:
            ...
            print(
                f"  {Theme.dim('usage')} {compact_usage(usage)} "
    ...
    def _handle_interactive_action(self, action: EnvAction, *, show_message: bool = True) -> None:
        ...
        if not (
            request.get("kind") == "response"
            and request.get("name") == "ask_human"
        ):
            return
    ...
    def _print_header(self) -> None:
        ...
        print_box_line(f"OpenETA Agent Console", width=width, strong=True)
...
def _build_cli_tool_registry(cli: OpenEtaCli) -> ToolRegistry:
    tools = build_default_tool_registry()
    bind_dummy_tool_handlers(
        tools,
        approve_world_mutating=lambda context: cli.confirm(
    ...
    tools.bind_handler("save_memory", lambda context: _save_memory_handler(cli, context))
    tools.bind_handler("get_memory", lambda context: _get_memory_handler(cli, context))
    tools.bind_handler("delete_memory", lambda context: _delete_memory_handler(cli, context))
    tools.bind_handler("compact_memory", lambda context: _compact_memory_handler(cli, context))
    tools.bind_handler("register_skill", lambda context: _register_skill_handler(cli, context))
    tools.bind_handler("update_skill", lambda context: _update_skill_handler(cli, context))
    _bind_perception_mcp_handlers(cli, tools)
    _bind_python_exec_handler(cli, tools)
    transport = _ensure_simulator_mcp_transport(cli)
    if transport is not None:
        bind_simulator_mcp_tool_handlers(
            tools,
            transport=transport,
            config=cli.state.simulator_mcp_config,
            response_callback=cli._sync_simulator_mcp_response,
            replace=True,
        )
    tools.add_listener(cli._print_tool_event)
    return tools
...
def _build_cli_checker_config(tools: ToolRegistry) -> CheckerSubagentConfig:
    ...
    raw_pre_checks = os.environ.get("OPENETA_PRE_SAFETY_CHECKS", "").strip()
    pre_safety_checks: dict[str, str] = {}
    if raw_pre_checks:
        try:
            parsed = json.loads(raw_pre_checks)
        ...
        for target, checker in parsed.items():
            if not isinstance(target, str) or not isinstance(checker, str):
                raise ValueError(
                    "OPENETA_PRE_SAFETY_CHECKS keys and values must be strings"
                )
            tools.get(target)
            ...
            pre_safety_checks[target] = checker
...
def _bind_perception_mcp_handlers(cli: OpenEtaCli, tools: ToolRegistry) -> None:
    sam3_url = _load_mcp_url("openeta-sam3", aliases=("sam3",))
    if sam3_url:
        tools.bind_handler(
            "sam3",
            build_sam3_handler(build_sse_sam3_mcp_segmenter(url=sam3_url)),
            replace=True,
        )
    anygrasp_url = _load_mcp_url("openeta-anygrasp", aliases=("anygrasp",))
    if anygrasp_url:
        tools.bind_handler(
            "anygrasp",
            build_anygrasp_handler(build_sse_anygrasp_mcp_grasper(url=anygrasp_url)),
            replace=True,
        )
    anyplace_url = _load_mcp_url("openeta-anyplace", aliases=("anyplace",))
    if anyplace_url:
        tools.bind_handler(
            "anyplace",
            build_anyplace_handler(build_sse_anyplace_mcp_placer(url=anyplace_url)),
            replace=True,
        )
    contact_graspnet_url = _load_mcp_url(
        "openeta-contact-graspnet",
        aliases=("contact-graspnet", "contact_graspnet"),
    )
    if contact_graspnet_url:
        tools.bind_handler(
            "contact_graspnet",
            build_contact_graspnet_handler(
                build_sse_contact_graspnet_mcp_predictor(url=contact_graspnet_url)
            ),
            replace=True,
        )


def _bind_python_exec_handler(cli: OpenEtaCli, tools: ToolRegistry) -> None:
    transport = _ensure_simulator_mcp_transport(cli)
    runtime = PythonExecRuntime(
        PythonExecConfig(
            mcp_transport=transport,
            allow_outside_sandbox=True,
            mcp_response_callback=cli._sync_simulator_mcp_response,
            approve_outside_sandbox=lambda context, mode: cli.confirm(
...
def _refresh_simulator_mcp_tool_catalog(cli: OpenEtaCli) -> None:
    ...
    if transport is None or not hasattr(transport, "list_tools"):
        return
    try:
        catalog = transport.list_tools(timeout_s=10.0)
    except Exception as exc:  # noqa: BLE001 - catalog discovery must be best effort.
        cli.state.simulator_mcp_tool_catalog = {
            "available": False,
            "error_type": type(exc).__name__,
            "message": str(exc),
            "url": cli.state.simulator_mcp_url,
        }
        return
    ...
    artifact = materialize_json_response(
        catalog,
        bundle_id="mcp-list_tools-cli",
        name="response",
    )
    cli.state.simulator_mcp_tool_catalog = _compact_mcp_tool_catalog(
        catalog,
...
def _compact_mcp_tool_catalog(
    ...
    *,
    ...
    return {
        ...
        "grep_hint": grep_hint,
        "tools": [_compact_mcp_tool_doc(tool) for tool in tools[:32] if isinstance(tool, dict)],
    }
...
def _save_memory_handler(cli: OpenEtaCli, context: ToolExecutionContext) -> ToolResult:
    ...
    if namespace == "artifacts":
        runtime.memory.save_artifact(key, payload, source=context.name)
    elif namespace == "skill_notes":
        runtime.memory.save_skill_note(key, payload, source=context.name)
    else:
        runtime.memory.save_fact(key, payload, source=context.name)
    runtime.update_memory(
        {
            "type": "tool_memory",
            "namespace": namespace,
    ...
    return ToolResult(
        ...
        details={"tool": context.name, "namespace": namespace, "key": key},
    )


def _get_memory_handler(cli: OpenEtaCli, context: ToolExecutionContext) -> ToolResult:
    runtime = cli._require_runtime()
    ...
    result = runtime.memory.get_memory(key_str or None, namespace=namespace)
    return ToolResult(True, content="memory loaded", details=result)


def _delete_memory_handler(cli: OpenEtaCli, context: ToolExecutionContext) -> ToolResult:
    runtime = cli._require_runtime()
    key = str(context.parameters.get("key", "")).strip()
    if not key:
        return ToolResult(False, content="delete_memory requires a key.")
    namespace = str(context.parameters.get("namespace", "all")).strip() or "all"
    deleted = runtime.memory.delete_memory(key, namespace=namespace)
    return ToolResult(True, content="memory deleted", details=deleted)


def _compact_memory_handler(cli: OpenEtaCli, context: ToolExecutionContext) -> ToolResult:
    runtime = cli._require_runtime()
    ...
    try:
        ...
        max_events = 8
    summary = runtime.memory.compact(max_events=max_events)
    return ToolResult(True, content=summary, details={"summary": summary})


def _register_skill_handler(cli: OpenEtaCli, context: ToolExecutionContext) -> ToolResult:
    runtime = cli._require_runtime()
    try:
        spec = _skill_spec_from_parameters(context.parameters)
        runtime.skills.register(spec)
    except (KeyError, ValueError) as exc:
        return ToolResult(False, content=str(exc), details={"tool": context.name})
    return ToolResult(
        True,
        content=f"skill registered: {spec.name}",
        details={"tool": context.name, "skill": spec.name},
    )


def _update_skill_handler(cli: OpenEtaCli, context: ToolExecutionContext) -> ToolResult:
    runtime = cli._require_runtime()
    try:
        existing = runtime.skills.get(str(context.parameters["name"]))
        spec = _skill_spec_from_parameters(
            {
                "description": existing.description,
                "content": existing.content,
                "allowed_tools": list(existing.allowed_tools),
                "task_patterns": list(existing.task_patterns),
                **context.parameters,
            }
        )
        runtime.skills.update(spec)
    except (KeyError, ValueError) as exc:
        return ToolResult(False, content=str(exc), details={"tool": context.name})
    return ToolResult(
        True,
        content=f"skill updated: {spec.name}",
        details={"tool": context.name, "skill": spec.name},
    )


def _skill_spec_from_parameters(parameters: JsonDict) -> SkillSpec:
    ...
    return SkillSpec(
        ...
        content=content,
        task_patterns=_string_tuple(parameters.get("task_patterns", ())),
        allowed_tools=_string_tuple(parameters.get("allowed_tools", ())),
        source="agent_cli",
...
def _memory_location(memory: AgentMemory) -> JsonDict:
    ...
    if isinstance(store, JsonMemoryStore):
        session_path = (
            str(store.session_path(memory.session_id))
    ...
    return {"root": "", "working_dir": "", "session_path": ""}
...
def _project_mapping(value: Any, keys: Iterable[str]) -> JsonDict:
    ...
    return {
        key: value[key]
        for key in keys
        if key in value and not _is_empty_param_value(value[key])
    }
...
def _tool_detail_width() -> int:
    columns = shutil.get_terminal_size(
        fallback=(TOOL_RESULT_FALLBACK_WIDTH, 24)
    ).columns
...
def main(argv: list[str] | None = None) -> int:
    ...
    cli = OpenEtaCli(model_override=args.model)
    try:
        if args.once:
            cli.run_task(
                args.once,
