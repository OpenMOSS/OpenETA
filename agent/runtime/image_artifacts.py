...
def materialize_mcp_images(
    ...
    *,
    ...
    resolved_bundle_id = safe_artifact_component(
        bundle_id or _new_bundle_id(),
        fallback="bundle",
    )
    root = artifact_session_root(output_root, session_id)
...
def _materialize_dict(
    ...
    *,
    ...
    for source_field, kind in _MCP_IMAGE_FIELDS.items():
        encoded = payload.get(source_field)
        if not isinstance(encoded, str) or not encoded.strip():
            continue
        fmt = _format_for_field(payload, source_field)
        ...
        payload[f"{source_field}_omitted"] = True
...
def _safe_filename(value: str) -> str:
    return safe_artifact_component(value, fallback="image")
