...
DEFAULT_ANYPLACE_DEPTH_TRUNCATION_M = 2.0
...
class PlannerContextConfig:
    ...
    anyplace_depth_truncation_m: float = DEFAULT_ANYPLACE_DEPTH_TRUNCATION_M

    def __post_init__(self) -> None:
        value = self.anyplace_depth_truncation_m
        if (
            ...
            raise ValueError("anyplace_depth_truncation_m must be finite and positive")
...
def _host_obligation_decision(
    ...
    *,
    ...
    if isinstance(point_fallback, dict):
        ...
        molmopoint_available = tools.can_execute("molmopoint")
        if (
            status == "required"
            and isinstance(parameters, dict)
            and molmopoint_available
        ):
            return PlannerDecision(
                action_type="tool_call",
                action="molmopoint",
        ...
        molmopoint_disabled = status == "required" and not molmopoint_available
        if status == "exhausted" or molmopoint_disabled:
            question = (
            ...
            escalation_status = "exhausted"
            if molmopoint_disabled:
                question = (
                    "Target localization failed after object-memory search and no "
                    "point-localization backend is configured; provide target "
                    "guidance or enable the pointing service."
                )
                reasoning = (
                    "Object-memory localization failed and the MolmoPoint fallback "
                    "is not available; stop instead of looping on an unreachable "
                    "perception backend."
                )
                escalation_status = "molmopoint_unavailable"
            return PlannerDecision(
                action_type="response",
                action="ask_human",
                parameters={
    ...
    if isinstance(execution, dict) and execution.get("status") == "required":
        ...
        if (
            ...
            return PlannerDecision(
                ...
                reasoning=(
                    "The attachment gate confirmed this candidate failed; dispatch "
                    "the exact reviewed recovery open before advancing the candidate queue."
                ),
                metadata={
                    "host_obligation": {
                        "schema_version": execution.get("schema_version"),
...
def _required_skill_inspection_name(
    ...
    *,
    ...
    if decision.action_type.lower().strip() != "tool_call":
        return ""
    if decision.action == "close_simulator_env":
        return ""
    if _matches_host_locked_action(decision, tool_context):
        return ""
    try:
...
def _matches_host_locked_action(decision: PlannerDecision, tool_context: JsonDict) -> bool:
    if decision.action_type.lower().strip() != "tool_call":
        return False
    execution = tool_context.get("grasp_execution")
    if isinstance(execution, dict):
        required = execution.get("required_action")
        if _decision_matches_required_action(decision, required):
            return True
        actions = execution.get("attachment_actions")
        if isinstance(actions, dict) and any(
            _decision_matches_required_action(decision, action)
            for action in actions.values()
        ):
            return True
    probe = tool_context.get("grasp_lift_probe")
    if isinstance(probe, dict):
        required_parameters = probe.get("required_parameters")
        if (
            probe.get("status") == "required"
            and decision.action == "move_to"
            and isinstance(required_parameters, dict)
            and decision.parameters == required_parameters
        ):
            return True
    return False


def _decision_matches_required_action(decision: PlannerDecision, action: object) -> bool:
    return (
        isinstance(action, dict)
        and decision.action == action.get("name")
...
def _validate_anyplace_parameters(parameters: JsonDict) -> list[str]:
    ...
    depth_truncation = parameters.get("depth_truncation")
    if depth_truncation is not None:
        if isinstance(depth_truncation, bool) or not isinstance(
            depth_truncation,
            (int, float),
        ):
            parsed_depth_truncation = float("nan")
        else:
            parsed_depth_truncation = float(depth_truncation)
        if (
            not math.isfinite(parsed_depth_truncation)
            or parsed_depth_truncation <= 0
        ):
            errors.append("anyplace `parameters.depth_truncation` must be finite and positive.")
...
def _default_tool_planner_system_prompt() -> str:
    return (
        ...
        "SAM3 prompt. "
        "A closed gripper is not proof of a successful grasp. When "
    ...
    if decision.action == "select_sam3_detection":
        ...
        if geometry_family and geometry_family not in {
            ...
            return [
                ...
                "drawer_handle, other, or unknown."
            ]
        return []
    ...
    *,
    ...
    if stage == "attachment":
        ...
        if any(
            ...
            return []
        return [
            "Attachment gate accepts only the exact full-lift action for PASS evidence "
            "or the exact reopen action after FAIL evidence. UNKNOWN requires observe."
        ]
    required = execution.get("required_action")
    ...
    *,
    ...
    return {
        ...
        "placement_obligation": _placement_obligation(
            ...
            retained=memory_context.get("retained_targeted_grasp"),
            memory_context=memory_context,
            depth_truncation_m=config.anyplace_depth_truncation_m,
        ),
...
def _placement_obligation(
    *,
    ...
    required = {
        ...
        "depth_truncation": float(depth_truncation_m),
    }
    return {
...
def _wrist_reference_obligation(
    *,
    ...
    active_task = memory_context.get("active_environment_task")
    task = str(active_task.get("task") or "") if isinstance(active_task, dict) else observation.task
    target_object = str(reference.get("target_object") or "").strip() or _exact_task_target_object(
        task,
        hint=no_detection.get("target_prompt"),
        memory_context=memory_context,
    )
    required = {
