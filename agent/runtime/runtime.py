...
class OpenEtaAgentRuntime:
    ...
    def _bind_memory_tool_handlers(self) -> None:
        handlers = {
            ...
            "select_grasp_candidate": self._select_grasp_candidate_tool,
    ...
    def _select_grasp_candidate_tool(
        self,
        context: ToolExecutionContext,
    ) -> ToolResult:
        candidate_id = str(context.parameters.get("candidate_id") or "").strip()
        reason = str(context.parameters.get("reason") or "").strip()
        try:
            selection = self.memory.select_grasp_candidate(
            ...
            return make_tool_result(
                ...
                success=False,
                content=str(exc),
                diagnostics=[{"code": "invalid_grasp_candidate_selection"}],
            )
        return make_tool_result(
            context,
            success=True,
            content=(
                f"Selected grasp candidate {candidate_id} as the active candidate."
            ),
