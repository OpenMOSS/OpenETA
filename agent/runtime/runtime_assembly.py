...
from agent.runtime.planner import (
    DEFAULT_ANYPLACE_DEPTH_TRUNCATION_M,
...
def assemble_runtime(config: RuntimeAssemblyConfig) -> RuntimeAssembly:
    ...
    planner = ToolCallingPlanner(
        ...
        context_config=PlannerContextConfig(
            ...
            anyplace_depth_truncation_m=config.anyplace_depth_truncation_m,
        ),
    )
...
def bind_runtime_perception_tools(
    ...
    *,
    ...
    grasp_backends = {}
    # All three grasp backends are enabled; the agent selects among them via the
    # grasp_pose_estimate fallback order. AnyGrasp candidates are gripper-agnostic;
    # GraspGenX samples for the robotiq_2f_85 geometry; Contact-GraspNet candidates
    # carry no jaw-height, so its handler injects a fixed placeholder
    # (CONTACT_GRASPNET_CANDIDATE_HEIGHT) to satisfy AnyPlace's placement schema.
    ...
    if endpoints.contact_graspnet_url:
        grasp_backends["contact_graspnet"] = build_contact_graspnet_handler(
            build_sse_contact_graspnet_mcp_predictor(
                url=endpoints.contact_graspnet_url
            ),
            output_root=artifact_root / "contact_graspnet_results",
        )
    if grasp_backends:
        tools.bind_handler(
            "grasp_pose_estimate",
