...
from agent.runtime.skills import BUILTIN_SKILL_DIR, SkillRegistry, SkillSpec
...
class SelfImprovementConfig:
    ...
    auto_apply_reviewed: bool = False
...
class SkillAutoApplier(Protocol):
    def apply(
        self,
...
class BackendReviewedSkillAutoApplier:
    ...
    def __init__(
        ...
        *,
        ...
        self.executable_tools = executable_tools
        ...
        *,
        ...
        request = SkillAuthoringRequest(
            ...
            parameters={
                ...
                "requested_changes": proposal.suggested_markdown,
                "requirements": proposal.rationale,
                "examples": [context.summary],
            },
...
class SelfImprovementReviewer:
    ...
    def __init__(
        ...
        *,
        ...
        auto_applier: SkillAutoApplier | None = None,
    ) -> None:
        ...
        *,
        ...
        for proposal in proposals:
            ...
            if self.config.auto_apply_reviewed:
                if self.auto_applier is None:
                    saved_proposal["auto_apply"] = {
                        "applied": False,
                        "reason": "independent reviewer is not configured",
                    }
                else:
                    try:
                        ...
                        if application.get("applied") is True:
                            applied = self.store.resolve_reviewed(
                                ...
                                resolution={"auto_apply": application},
                            )
                            saved_proposal = applied
                            saved_proposal["auto_apply"] = application
                        else:
                            saved_proposal["auto_apply"] = application
                    except Exception as exc:  # noqa: BLE001 - auto-apply fails closed.
                        saved_proposal["auto_apply"] = {
                            "applied": False,
                            "reason": str(exc),
    ...
    def _maybe_extract_task_playbook(self, result: Any, *, summary: JsonDict) -> JsonDict | None:
        signals = summary.get("signals")
        if not isinstance(signals, dict) or signals.get("positive_reward") is not True:
            return None
        if self.config.task_playbook_candidate_root is None or self.config.rollout_root is None:
            return None
        session_id = str(result.session_id or "").strip()
        if not session_id:
            return None
        rollout_path = (
            Path(self.config.rollout_root)
            / "sessions"
            / session_id
            / "rollout"
            / "tool_calls.jsonl"
        ...
        if not rollout_path.is_file():
            return {
                "created": False,
                "reason": "rollout tool-call ledger is unavailable",
            }
        episode = result.to_dict()
        scope_metadata = _episode_scope_metadata_from_result(episode)
        assistance = result.metadata.get("assistance")
        assistance = assistance if isinstance(assistance, dict) else {}
        outcome: JsonDict = {
            "episode_id": str(scope_metadata.get("episode_id") or session_id),
            "env_id": str(scope_metadata.get("env_id") or ""),
            "session_id": session_id,
            "status": "success",
            "assistance": {
                "assisted": bool(
                    assistance.get("agent_assisted")
                    or assistance.get("human_assisted")
                )
            },
            "episode": episode,
        }
        try:
            candidate = extract_task_playbook_candidate(
                outcome=outcome,
                rollout_tool_calls=rollout_path,
            )
            review = review_task_playbook_candidate(candidate, outcome=outcome)
            if not review.get("approved"):
                return {"created": False, "review": review}
            path = write_task_playbook(
                candidate,
                root=Path(self.config.task_playbook_candidate_root),
            )
        except (OSError, json.JSONDecodeError, TaskPlaybookError, ValueError) as exc:
            return {
                "created": False,
                "reason": str(exc),
                "error_type": type(exc).__name__,
...
def _episode_scope_metadata_from_result(episode: JsonDict) -> JsonDict:
...
def write_session_skill(
    ...
    *,
    ...
    """Atomically persist one validated SkillSpec inside a session-owned directory."""

    target = _skill_markdown_path(skill.name, skill_dir=Path(skill_dir))
    ...
    lines = [
        ...
        f"editable: {'true' if skill.editable else 'false'}",
        "task_patterns:",
        *[f"  - {value}" for value in skill.task_patterns],
        "allowed_tools:",
        *[f"  - {value}" for value in skill.allowed_tools],
        "---",
        "",
        skill.content.strip(),
        "",
