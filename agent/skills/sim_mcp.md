...
required MCP tool. It is not an AnyGrasp candidate rejection: do not retry the
...
If a perception or grasp MCP returns a camera-frame pose/candidate, treat it as
OpenCV camera frame unless the tool result explicitly says otherwise. Call
`camera_pose_to_world` with that pose and the corresponding camera calibration
from the latest simulator render/observation before calling a world-frame
control tool such as `move_to`. `camera_pose_to_world` understands the current
MuJoCo `pos + mat` convention and converts between OpenCV grasp poses and
OpenGL simulator camera axes when needed. Do not send camera-frame poses
directly to simulator control tools.
...
For AnyGrasp candidates, preserve the complete world-frame result from
`camera_pose_to_world` and pass it directly to one atomic `move_to` call. Do not
invent an intermediate pose or numerically adjust the transformed target. Close
the gripper with a separate `gripper_control` call after the motion succeeds.
