...
Vector-environment helpers are owned by the corresponding `sim/envs` backend.
Each simulator family still needs dependency-specific acceptance testing
before its vector execution path can be considered ready.
...
Provider calls, tool execution, turn lifecycle, and session ownership remain
separate interfaces so optional policy backends cannot bypass runtime safety
