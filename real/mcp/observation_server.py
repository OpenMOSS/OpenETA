...
Control tools issue physical motion after ``reset_env`` homes the arm:
``step_env`` applies relative Cartesian deltas, ``move_to`` sends absolute
Cartesian targets, and ``gripper_open``/``gripper_close`` actuate the Robotiq
gripper. Per-command translation and rotation caps reject oversized arm moves.
``base_control`` is unsupported because the UR5e is fixed-base.
...
def reset_env(handle: str = "", *, seed: int | None = None,
    ...
    """Home the arm, unlock control tools, and return the initial observation.

    This issues physical UR5e motion when ``home_joints`` is configured. If no
    home joints are configured, homing is a no-op, but the control gate is still
    unlocked. Returns top-level ``task, cameras, robot, objects, metadata``;
    cameras carry base64 PNG frames.
    """
...
def move_to(handle: str = "", x: float = 0.0, y: float = 0.0, z: float = 0.0, *,
    ...
    ``x,y,z`` are world metres and ``roll,pitch,yaw`` are degrees. When all
    orientation fields are omitted, the current orientation is preserved. If
    any are supplied, omitted components are treated as zero degrees. The net
    move from the current EEF pose is capped for safety; an over-cap request is
    REJECTED, never clamped.

    ``num_steps``, ``tolerance``, ``ori_tolerance``, and
    ``enable_collision_check`` are accepted for simulator-schema compatibility
    but are not implemented by this backend. In particular, this tool does not
    perform collision checking. Returns ``{target, start, end, steps_executed,
...
def base_control(forward: float = 0.0, lateral: float = 0.0, yaw: float = 0.0,
                 ...
                 session_id: str = "") -> dict[str, Any]:
    """Unsupported on the fixed-base UR5e; always returns an error without motion."""
...
def _build_env_factory(args: argparse.Namespace):
    ...
    def factory():
        ...
        # RealEnvManager.reset_env performs the explicit home. Keep the direct
        # environment reset observation-first to avoid duplicate homing.
...
def main() -> int:
    ...
    parser.add_argument("--config", default="",
                        help="Path to a local JSON hardware config "
                        "(copy real/config/ur5e_bench.example.json). "
                        "When set, the flat --camera-*/--robot-* flags are ignored.")
    parser.add_argument(
        "--robot-ip",
        default="",
        help="UR5e controller IP for RTDE state and physical motion control.",
    )
