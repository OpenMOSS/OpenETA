...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--anyplace-depth-truncation", type=float)
...
def _build_config(name: str, args: argparse.Namespace) -> ServiceConfig:
    ...
    if name == "anyplace":
        ...
        depth_truncation = args.anyplace_depth_truncation
        if depth_truncation is None:
            raw_depth_truncation = os.environ.get("OPENETA_ANYPLACE_DEPTH_TRUNCATION")
            if raw_depth_truncation:
                depth_truncation = float(raw_depth_truncation)
        command = [
            python,
        ...
        if depth_truncation is not None:
            command.extend(["--depth-truncation", str(depth_truncation)])
        return ServiceConfig(
            name=name,
            python=python,
            host=args.host,
