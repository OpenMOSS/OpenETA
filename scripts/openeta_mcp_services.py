...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "target",
        choices=("sam3", "anygrasp", "anyplace", "contact_graspnet", "all"),
    )
...
def _build_configs(args: argparse.Namespace) -> list[ServiceConfig]:
    targets = (
        ("sam3", "anygrasp", "anyplace", "contact_graspnet")
        if args.target == "all"
        else (args.target,)
    )
...
def _status_service(config: ServiceConfig) -> dict[str, Any]:
    ...
    health_ok = _http_health_check(config.health_url, expected_server=config.name)
    return {
...
async def _mcp_list_tools_async(url: str) -> list[str]:
    from agent.tools.sim_mcp import _temporary_no_proxy_for_url
    from mcp import ClientSession
    from mcp.client.sse import sse_client

    with _temporary_no_proxy_for_url(url):
...
def _wait_for_service_ready(
    ...
    while time.monotonic() < deadline:
        ...
        if _http_health_check(config.health_url, expected_server=config.name):
            return None
        time.sleep(0.1)
    return {
        "ok": False,
