...
from sim.envs import SupportedEnvType


# ── Lightweight SupportedModel stub (replaces rlinf.config.SupportedModel) ─
# In the extracted env layer we don't need the full model-registry infra;
# a plain string-based sentinel is sufficient for the action-remapping
# functions below.

class _ModelStub:
    """String-keyed sentinel that behaves like ``SupportedModel``."""
    __slots__ = ("value",)
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, _ModelStub):
    ...
    def __repr__(self) -> str:
        return f"SupportedModel({self.value!r})"


class SupportedModel:
    """Minimal stand-in for ``rlinf.config.SupportedModel``."""

    OPENVLA = _ModelStub("openvla")
    OPENVLA_OFT = _ModelStub("openvla_oft")
    OPENPI = _ModelStub("openpi")
    GR00T_N1D6 = _ModelStub("gr00t_n1d6")
    GR00T_N1D7 = _ModelStub("gr00t_n1d7")

    _known: dict[str, _ModelStub] = {
        "openvla": OPENVLA,
    ...
    def __new__(cls, value: str) -> _ModelStub:
        if value not in cls._known:
            cls._known[value] = _ModelStub(value)
