...
def _rotation_matrix_to_quat_xyzw(matrix: Any) -> np.ndarray | None:
    """Convert a finite 3x3 rotation matrix to a normalized xyzw quaternion."""
...
class UnifiedEnv(gym.Env):
    ...
    def _normalise_maniskill(self, raw: dict) -> dict:
        ...
        # ``robot.get_pose()`` is the *base* link pose (for a fixed Panda it
        # never moves), which makes closed-loop EE control blind.  Use the
        # controller's TCP link (``*_tcp``) when one exists.  SAPIEN poses use
        # wxyz quaternions; the unified contract is xyzw.
        try:
            inner = self._unwrap()
            robot = getattr(getattr(inner, "agent", None), "robot", None)
            if robot is not None:
                tcp_pose = None
                links_map = getattr(robot, "links_map", {}) or {}
                for link_name, link in links_map.items():
                    if str(link_name).endswith("tcp"):
                        tcp_pose = getattr(link, "pose", None)
                        break
                if tcp_pose is None and hasattr(robot, "get_pose"):
                ...
                if tcp_pose is not None:
                    ...
                    obs.setdefault("proprio", {})["ee_pose"] = np.concatenate(
                        [pos, quat_xyzw]
                    )
                # Root (base) pose for world→base frame conversion of
                # Cartesian deltas.  The pd_ee_delta_pose controller consumes
                # position deltas in the root frame, and the MCP move_to tool
                # needs base_pose.quat_xyzw to rotate world-frame commands.
                if hasattr(robot, "get_pose"):
                    root_pose = robot.get_pose()
                    if root_pose is not None:
                        root_pos = self._np(root_pose.p).flatten()[:3]
                        root_quat_wxyz = self._np(root_pose.q).flatten()[:4]
                        obs.setdefault("proprio", {})["base_pose"] = {
                            "xyz": root_pos.tolist(),
                            "quat_xyzw": np.concatenate(
                                [root_quat_wxyz[1:4], root_quat_wxyz[0:1]]
                            ).tolist(),
                        }
        except Exception:
            pass
    ...
    def _normalise_libero(self, raw: dict) -> dict:
        ...
        if eef_pos is not None:
            ep = self._np(eef_pos)
            site_quat = self._extract_libero_grip_site_quat_xyzw()
            if site_quat is not None:
                proprio["ee_pose"] = np.concatenate([ep, site_quat])
                orientation_frame = "grip_site"
            else:
                # Position-only is intentionally safer than publishing the
                # known-incompatible right_hand orientation as a coherent EEF
                # pose.  Orientation-controlled move_to will fail closed when
                # the grip-site matrix is unavailable.
                proprio["ee_pose"] = ep
                orientation_frame = None
            proprio["metadata"] = {
                "ee_position_frame": "grip_site",
                "ee_orientation_frame": orientation_frame,
                "ee_pose_frame_consistent": orientation_frame == "grip_site",
            }
            if eef_quat is not None:
                proprio["metadata"]["right_hand_quat_xyzw"] = (
                    self._np(eef_quat).reshape(-1)[:4].tolist()
                )
        if gripper is not None:
            ...
            gripper_magnitude = float(np.abs(g).mean()) if g.size > 0 else 0.0
            proprio["gripper_open"] = min(1.0, gripper_magnitude * 25.0)
            # Keep the native values in the structured channel as well.  The
            # derived scalar is convenient for generic agents, but cannot be
            # used to reconstruct the actual gripper state during debugging.
            proprio["gripper_qpos"] = g
            pad_geometry = self._extract_libero_finger_pad_geometry()
            if pad_geometry:
                proprio["gripper_geometry"] = pad_geometry
        gripper_vel = raw.get("robot0_gripper_qvel")
        if gripper_vel is not None:
            proprio["gripper_qvel"] = self._np(gripper_vel)
    ...
    def _extract_libero_finger_pad_geometry(self) -> dict[str, Any]:
        """Return live Panda finger-pad contact geometry in world coordinates.

        The gripper joint aperture is not the distance between the MuJoCo pad
        geom centers, and the pad contact plane is slightly offset from
        ``grip_site`` along the approach axis.  Publish the compiled, live
        robot geometry so downstream visualization does not have to guess
        either relationship.
        """
        ...
        if model is None or data is None:
            return {}
        try:
            grip_site_id = int(model.site_name2id("gripper0_grip_site"))
            grip_site = np.asarray(data.site_xpos[grip_site_id], dtype=np.float64)
            grip_rotation = np.asarray(
                data.site_xmat[grip_site_id], dtype=np.float64
            ).reshape(3, 3)
        except Exception:
            return {}
        ...
        geom_centers: list[list[float]] = []
        geom_rotations: list[list[list[float]]] = []
        contact_centers: list[list[float]] = []
        contact_offsets: list[list[float]] = []
        ...
        for name in pad_names:
            try:
                ...
                return {}
            toward_grip = grip_site - center
            norm = float(np.linalg.norm(toward_grip))
            if (
                center.shape != (3,)
                ...
                or not np.isfinite(size[:3]).all()
                or norm <= 1e-12
            ):
                return {}
            direction = toward_grip / norm
            # The Panda pad collision geom is a box. Its support distance in
            # an arbitrary world direction is the sum of projected half-sizes.
            support = float(
                np.sum(np.abs(rotation.T @ direction) * size[:3])
            )
            contact = center + direction * support
            geom_centers.append(center.tolist())
            geom_rotations.append(rotation.tolist())
            contact_centers.append(contact.tolist())
            contact_offsets.append(
                (grip_rotation.T @ (contact - grip_site)).tolist()
            )
            half_sizes.append(size[:3].tolist())

        return {
            "source": "live_mujoco_collision_geometry",
            "eef_frame": "panda_grip_site",
            "finger_pad_geom_names": list(pad_names),
            "finger_pad_geom_centers_world_m": geom_centers,
            "finger_pad_geom_rotations_world": geom_rotations,
            "finger_pad_inner_contact_centers_world_m": contact_centers,
            "finger_pad_inner_contact_offsets_grip_site_m": contact_offsets,
            "finger_pad_geom_half_sizes_m": half_sizes,
        }

    def _extract_libero_mujoco_contacts(self) -> dict[str, Any] | None:
        """Return current MuJoCo contacts at or near the Panda grip site."""

        model = self._find_mj_model()
        data = self._find_mj_data()
        if model is None or data is None or not hasattr(data, "contact"):
            return None
        try:
            contact_count = max(0, int(data.ncon))
            grip_site_id = int(model.site_name2id("gripper0_grip_site"))
            grip_site = np.asarray(
                data.site_xpos[grip_site_id],
                dtype=np.float64,
            ).reshape(3)
        except Exception:
            return None
        ...
        def geom_name(geom_id: int) -> str:
            for method_name, args in (
                ("geom_id2name", (geom_id,)),
                ("id2name", (geom_id, "geom")),
            ):
        ...
        for index in range(contact_count):
            try:
                ...
                position = np.asarray(contact.pos, dtype=np.float64).reshape(3)
                first_name = geom_name(int(contact.geom1))
                second_name = geom_name(int(contact.geom2))
            except Exception:
                continue
            if not np.isfinite(position).all():
                continue
            distance_m = float(np.linalg.norm(position - grip_site))
            robot_geom = any(
                name.startswith(("robot0_", "gripper0_"))
                for name in (first_name, second_name)
            )
            if not robot_geom and distance_m > 0.12:
                continue
            selected.append(
                (
                    distance_m,
                    {
                        ...
                        "geom_names": [first_name, second_name],
                    },
                )
            )

        # MuJoCo may emit several almost-identical constraint points for one
        # physical patch. Keep only the current patch centers needed by the
        # visual marker; never accumulate contacts across simulator steps.
        points: list[dict[str, Any]] = []
        for _, candidate in sorted(selected, key=lambda item: item[0]):
            position = np.asarray(candidate["position_xyz_m"], dtype=np.float64)
            if any(
                float(
                    np.linalg.norm(
                        position
                        - np.asarray(existing["position_xyz_m"], dtype=np.float64)
                    )
                )
                < 0.004
    ...
    def _extract_libero_grip_site_quat_xyzw(self) -> np.ndarray | None:
        """Read the OSC-controlled LIBERO grip-site orientation from MuJoCo.

        Legacy robosuite reports ``robot0_eef_pos`` from ``grip_site`` but
        ``robot0_eef_quat`` from the physical hand body.  Those frames differ
        by a fixed rotation on Panda, so combining them creates a pose that no
        single physical frame actually has.  The OSC controller itself uses
        ``eef_site_id``; read that same site's matrix for closed-loop feedback.
        """
        ...
        try:
            ...
            if sim is None or not robots:
                return None
            site_id = getattr(robots[0], "eef_site_id", None)
            if site_id is None:
                return None
            site_matrix = np.asarray(
                sim.data.site_xmat[int(site_id)], dtype=np.float64
            ).reshape(3, 3)
        except (AttributeError, IndexError, TypeError, ValueError):
            return None
        return _rotation_matrix_to_quat_xyzw(site_matrix)
    ...
    def step(self, action: Any
        ...
        obs = self._normalise_obs(raw_obs)
        if self._backend == "libero":
            contacts = self._extract_libero_mujoco_contacts()
            if contacts is not None:
                obs.setdefault("metadata", {})["mujoco_contacts"] = contacts
    ...
    def check_success(self) -> bool | None:
        """Forward a simulator-native checker when the backend provides one."""
        checker = getattr(self._env, "check_success", None)
    ...
    def get_sim_state(self) -> Any:
        """Forward backend state for host/evaluator diagnostics."""
