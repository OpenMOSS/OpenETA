...
def test_batch_worker_factory_binds_graspgenx_behind_unified_tool(
    ...
    def fake_grasp_pose_estimate_handler(handlers, **kwargs):
        assert kwargs == {"graspgenx_gripper_name": "robotiq_2f_85"}
        facade_backends.update(handlers)
        return lambda _context: ToolResult(True, content="predicted")

    monkeypatch.setattr(
        runtime_assembly,
        "build_grasp_pose_estimate_handler",
        fake_grasp_pose_estimate_handler,
    )
