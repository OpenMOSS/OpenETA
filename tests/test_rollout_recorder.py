...
def test_rollout_externalizes_provider_images_and_redacts_secrets(tmp_path: Path) -> None:
    ...
    backend = OpenAICompatiblePlannerBackend(
        OpenAICompatiblePlannerBackendConfig(
            ...
            api_key="test-api-key-must-never-be-recorded",
            enable_vision=True,
        ),
        transport=transport,
    )
    ...
    recorder.start_session(
        session_id="vision",
        task="inspect",
        provenance={"api_key": "test-api-key-must-never-be-recorded"},
    )
    now = time.time()
    ...
    assert "test-api-key-must-never-be-recorded" not in text
    ...
    assert captured["headers"]["Authorization"].endswith(
        "test-api-key-must-never-be-recorded"
    )
