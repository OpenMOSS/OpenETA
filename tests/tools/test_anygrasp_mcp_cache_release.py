...
class _Backend:
    ...
    def service_info(self) -> dict[str, Any]:
        return {"openeta": {"revision": "abc123", "dirty": False}}
...
def test_health_payload_reports_backend_provenance(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(anygrasp_mcp_server, "_BACKEND", _Backend())

    assert anygrasp_mcp_server.health_payload() == {
        "ok": True,
        "server": "anygrasp",
        "backend_configured": True,
        "service": {"openeta": {"revision": "abc123", "dirty": False}},
    }
