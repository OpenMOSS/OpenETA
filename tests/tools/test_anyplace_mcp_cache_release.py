...
class _Backend:
    ...
    def predict_placement(self, **kwargs: Any) -> dict[str, Any]:
...
def test_predict_placement_releases_cache_after_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = anyplace_mcp_server.predict_placement(**_request(), depth_truncation=2.0)

    assert result == {"success": True}
    assert backend.calls[0]["depth_truncation"] == 2.0
