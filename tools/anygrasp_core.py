...
class AnyGraspBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self._service_info = {
            ...
            "openeta": _git_identity(_REPO_ROOT),
            "anygrasp_sdk": _git_identity(self.sdk_root),
    ...
    def service_info(self) -> dict[str, Any]:
        """Return immutable startup provenance for health and result records."""

        return copy.deepcopy(self._service_info)
...
def _git_identity(root: Path) -> dict[str, Any]:
    identity: dict[str, Any] = {"revision": None, "dirty": None}
    try:
        ...
        return identity
    identity["revision"] = revision or None
    identity["dirty"] = bool(status.strip())
    return identity


def _checkpoint_identity(path: Path) -> dict[str, Any]:
    identity: dict[str, Any] = {"name": path.name, "sha256": None, "size_bytes": None}
    try:
        stat = path.stat()
    except OSError:
        return identity
